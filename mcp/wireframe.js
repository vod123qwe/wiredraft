...
function renderComponent(b, grid, o, c, r, w, h) {
  ...
  switch (ct) {
    ...
    case 'listitem': { const lead = o.icon || '●'; const mid = r + Math.floor((h - (o.body ? 1 : 0)) / 2); writeText(b, grid, c, mid, (lead + '  ' + (lab || 'Title')).slice(0, w - 2)); if (o.body) writeText(b, grid, c + 3, mid + 1, String(o.body).slice(0, w - 5)); put(b, grid, c + w - 1, mid, '›'); for (let i = 0; i < w; i++) put(b, grid, c + i, r + h - 1, '─'); return; }
